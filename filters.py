...
def create_filters(**filters):
    ...
    Each of these arguments is provided by the main modul
     with a value from the
    user's options at the command line. Each one corresponds
    to a different type
    of filter. For example, the `--date` option correspond
     to the `date`
    argument, and represents a filter that selects close
    approaches that occured
    on exactly that given date. Similarly, the
    `--min-distance` option
    ...
    because the main module directly passes this result
    to that method. For now,
