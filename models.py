...
class CloseApproach:
    ...
    def __init__(self, **info):
        ...
        """
        # TODO: Assign information from the arguments passed to the constructor
        # onto attributes named `designation`, `time`, `distance`, and `velocity`.
        # You should coerce these values to their appropriate data type and handle any edge cases.
        # The `cd_to_datetime` function will be useful.
        self.designation = '' if 'designation' not in info else str(info['designation'])
        self.time = None if 'time' not in info else cd_to_datetime(info['time'])  # TODO: Use the cd_to_datetime function for this attribute.
        self.distance = 0.0 if 'distance' not in info else float(info['distance'])
        self.velocity = 0.0 if 'velocity' not in info else float(info['velocity'])
